...
# desc: X-masmatch is a small Christmas-themed card matching game.
# site: https://github.com/son-link/x-masmatch
...
class HallowenMatch():
    def __init__(self):
        ...
        pyxel.init(WITH, HEIGHT, title='X-masmatch', display_scale=3,
    ...
    def draw(self):
        ...
        if self.game_state == STATE_INIT:
            ...
            self.centerText('Mouse: left click to flip card', 86, 9, self.font)
            self.centerText('Keyboard: Move: arrow keys. Z: Select', 100, 9, self.font)
            self.centerText('Gamepad: Move: D-PAD. A: Select', 114, 9, self.font)
    ...
    def update(self):
        ...
        if self.game_state == STATE_INIT:
            ...
            if (
                self.flip_down and
                len(self.matchs) > 0 and
                len(self.selected) > 0 and
                self.frame_count > self.flip_down and
                (self.frame_count - self.flip_down) % 60 == 0
            ):
